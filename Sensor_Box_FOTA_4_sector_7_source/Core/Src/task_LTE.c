...
{
  HAL_UART_Transmit(&huart3, (uint8_t *)command, (uint16_t)strlen(command), 1000);
...
void CheckSimConditionCommand(int index)
{
  ...
  switch(index)
  {
    ...
    case 5:
    {
      ...
      HAL_UART_Transmit(&huart3, (uint8_t *)dynamicCommand, (uint16_t)strlen(dynamicCommand), 1000);
      break;
    }
    case 6:
    {
...
{
  sendSMSNotification("Firmware updated successfully");
}

void sendFailNotification()
{
  //sendSMSNotification("Firmware updated failed");
}

void sendUserWarning()
{
  //sendSMSNotification("There is an unregisted user try giving command to your system. Please registed if it is a new user");
}

void sendSMSNotification(char* message)
{
  char dynamicCommand[200];
  SendCommandToSim("AT+CMGF=1");
  deleteBuffer(dynamicCommand);
  sprintf(dynamicCommand, "AT+CMGS=\"%s\"\r\n", PhoneNumber);
  HAL_UART_Transmit(&huart3, (uint8_t *)dynamicCommand, (uint16_t)strlen(dynamicCommand), 1000);
  deleteBuffer(dynamicCommand);
  sprintf(dynamicCommand, "%s\r\n\x1A", message);
  HAL_UART_Transmit(&huart3, (uint8_t *)dynamicCommand, (uint16_t)strlen(dynamicCommand), 1000);
}